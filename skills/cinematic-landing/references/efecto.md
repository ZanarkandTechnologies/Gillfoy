# Efecto Integration: ASCII & Visual Effects

> **When to use**: Adding ASCII, dither, or glitch effects to your landing page using [efecto.app](https://efecto.app/).

## Workflow

1. **Design in Efecto**: Go to [efecto.app](https://efecto.app/) and configure your effect
2. **Export Code**: Click the export button to get React Three Fiber code
3. **Integrate**: Copy the exported code into your project (template below)
4. **Customize**: Adjust uniforms to match your scene

## Why Efecto?

- Visual effect designer with **live preview**
- Exports production-ready R3F code
- Effects: ASCII, Dither, Glitch, CRT, and more
- No shader knowledge required

## Alternative: Tooooools

[Tooooools.app](https://www.tooooools.app/) is great for **static image effects**:
- Stippling, dots, patterns
- Dithering, ASCII
- Edge detection, distortion
- CRT, cellular automata

**Workflow**: Upload image → Apply effect → Export as image/video

Use Tooooools when you want to **bake the effect into your assets** (static).  
Use Efecto when you want **real-time WebGL effects** (dynamic).

---

## Example Export (ASCII Effect)

The code below is an example export from Efecto. Replace with your own export.

**Note:** This code export provides a faithful representation of the effect you configured in Efecto.
However, visual results may vary slightly in your implementation due to differences in:
- Scene setup and lighting
- Canvas resolution and aspect ratio
- Animation timing and frame rates
- Three.js/postprocessing library versions

The shader code is identical to what Efecto uses, so with similar scene conditions,
you should get very close results. Feel free to adjust the uniform values to fine-tune the look.


Please create the following files with the exact code provided:
...
## File 1: components/ascii-effect.tsx
```tsx
"use client"
...
## File 2: components/effect-scene.tsx
```tsx
"use client"
...
your-project/
├── components/
│   ├── ascii-effect.tsx    ← Copy "Effect Component" code here
│   └── effect-scene.tsx     ← Copy "Scene Component" code here
└── app/
    └── page.tsx            ← Use it here
...
// That's it! The import path "./ascii-effect" in effect-scene.tsx
// will correctly resolve since both files are in the same folder.

Please create these files and let me know if you need any clarification about the implementation.